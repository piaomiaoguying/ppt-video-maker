...
在项目根目录下启动 **Claude Code**，输入斜杠命令即可启动全流程：
...
```
/ppt-video-maker 帮我调研一下XXX
```

AI 会全自动完成：**联网搜索调研 → 生成调研资料 MD → 生成 HTML PPT → 生成 TTS 发言稿 → 生成音频 → 生成 Remotion 视频**。

![Claude Code 启动界面](使用方法截图1.jpg)

![输入斜杠命令](使用方法截图2.png)

> 只需一句话描述主题，全程无需手动操作，等待即可得到完整视频。

如果你已经有现成的调研材料，也可以直接提供 Markdown 文件作为素材来源，AI 会基于已有材料直接进入 PPT 生成和视频制作阶段，跳过联网调研：

```
/ppt-video-maker 调研资料.md 用这份材料制作
```

![使用已有材料](使用方法截图3.png)

### 手动启动 Remotion Studio
